 ...
 * 
 */
public final class Ciena_Juniper_A_Z_AlarmProcessing extends JuniperLinkDown_ProblemDefault implements
		ProblemInterface {

	private static final String LINKDOWN_KEY = "50003/100/1";
	
	private Logger log = LoggerFactory.getLogger(Ciena_Juniper_A_Z_AlarmProcessing.class);

	public Ciena_Juniper_A_Z_AlarmProcessing() {
		super();
	...
	public boolean isMatchingTriggerAlarmCriteria(Alarm a) throws Exception {
		...
		if ( a instanceof EnrichedJuniperAlarm)  {
			...
			if(a.getCustomFieldValue(GFPFields.EVENT_KEY).equals(LINKDOWN_KEY)) {
				if(alarm.getRemoteDeviceType() != null &&
						!alarm.getRemoteDeviceType().isEmpty() &&
						alarm.getRemoteDeviceType().equals("CIENA NTE")) {	

					// the ciean LD is delayed by the ciena VP for one minute so we may have to wait here for it to arrive
					Util.setTriggerWatch(alarm, EnrichedJuniperAlarm.class, "simpleSendCallBack", 90000, "Ciena/Juniper A-Z 2 sec watch dog.");
					//setTriggerWatch(alarm);
					ret = true;
				} 
			} else {
				Util.setTriggerWatch(alarm, EnrichedJuniperAlarm.class, "simpleSendCallBack", 2000, "Ciena/Juniper A-Z 2 sec watch dog.");
				ret = true;
			}
	...
	public void whatToDoWhenSubAlarmIsAttachedToGroup(Alarm alarm, Group group) throws Exception {
		...
		// test to see if this is the trigger.  If the Ciena link down alarm is to be
		// suppressed or cascade the alarm
		if(alarm != trigger ) {
			// the second alarm that comes in is suppressed no matter which one it is
			if(Long.parseLong(eAlarm.getCustomFieldValue(GFPFields.BE_ALARM_TIME_STAMP)) > 
			Long.parseLong(trigger.getCustomFieldValue(GFPFields.BE_ALARM_TIME_STAMP))) {
				eAlarm.setSuppressed(true);
				log.info("Alarm " + eAlarm.getIdentifier() + " sequence:" + eAlarm.getCustomFieldValue(GFPFields.SEQNUMBER)
						+ " has been suppressed by A_Z_AlarmProcessing.");
			} else {
