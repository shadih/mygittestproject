...
//
public final class CienaLD_Suppression extends ProblemDefault implements ProblemInterface 
{

	private Logger log = LoggerFactory.getLogger(CienaLD_Suppression.class);
	public CienaLD_Suppression() {
		super();
	...
	public List<String> computeProblemEntity(Alarm a) throws Exception
	{
		...
		problemEntities.add(alarm.getDevice_ip());
	...
	public boolean isMatchingTriggerAlarmCriteria(Alarm a) throws Exception {
		boolean ret = false;
		CienaAlarm alarm = (CienaAlarm) a;
		if ("CIENA EMUX".equals(alarm.getDeviceType()) && !alarm.isSuppressed())
		{
			...
			String watchdogDesc = "CienaLD watch dog.";
...
", alarm be_time_stamp = " + alarm.getCustomFieldValue("be_time_stamp"));
	...
	public void whatToDoWhenSubAlarmIsAttachedToGroup(Alarm a, Group group) throws Exception {
		...
		{
			log.info("alarm = " + trigger.getIdentifier() + " is suppressed due to existence of 50002/100/11 or 50002/100/14.");
