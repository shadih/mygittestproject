...
public class SAREAExtendedLifeCycle extends LifeCycleAnalysis {
	...
	private static Logger log = LoggerFactory.getLogger(SAREAExtendedLifeCycle.class);

	public SAREAExtendedLifeCycle(Scenario scenario) {   
		super(scenario);
	...
	public void processIncomingData(AlarmCommon inData)
	{
		...
		try {
			...
			if (log.isTraceEnabled()) {
				log.trace("processIncomingData: ERROR:" + e.toString() + " Trace = " 
	...
	private void enrichTopologyInfo(EnrichedPreprocessAlarm enrichedAlarm) {
		...
		// comment below if at line 233 for SAREA
		...
		if ( !enrichedAlarm.isSuppressed() ) {
			...
			// comment checkIfsubscribedToCDC(enrichedAlarm) below for SAREA
			...
			// comment setHairPinIndicator(enrichedAlarm) below for SAREA
	...
	private void setAsPassthru(EnrichedPreprocessAlarm enrichedAlarm) {
		...
		if("50001/100/50".equalsIgnoreCase(enrichedAlarm.getCustomFieldValue(NetcoolFields.EVENT_KEY)) && "DEVICE".equalsIgnoreCase(enrichedAlarm.getOrigMEClass())) {
			if (log.isTraceEnabled()) {
				log.trace("setting 50001/100/50 DEVICE level alarm as passthru" + enrichedAlarm.getIdentifier());
			}
			enrichedAlarm.setPassthru(true); 
		}    
	...
	private void getAllTopologyInformation(EnrichedPreprocessAlarm alarm) {
		...
		// update below for SAREA, always set managedObjectInstance to device IP address
		//String origManagedObjectInstance = alarm.getOriginatingManagedEntity().split(" ")[1];
		String managedObjectInstance = alarm.getOriginatingManagedEntity().split(" ")[1];
		//String managedObjectInstance = origManagedObjectInstance.split("/")[0];
		String managedObjectClass = alarm.getOriginatingManagedEntity().split(" ")[0];
		//String managedObjectClass = "DEVICE";
		...
		if(!alarm.isSuppressed()) {
		    ...
		    // 	Feb18 294728b - SAREA Access as a Service on Domain 2
			// "PLANNED" provStatus shall be ignored
			if (null != alarm.getProvStatus() && ! alarm.getProvStatus().isEmpty()) {
				String status = alarm.getProvStatus();
				if ( status.equals("PLANNED") ) {
					...
					if(log.isTraceEnabled()) {
						log.trace("Suppressing alarm: " + alarm.getIdentifier() + " with SeqNum: " + alarm.getCustomFieldValue(GFPFields.SEQNUMBER) +
								" since Device has provision_status = "+status+".");
					}
				}
				else if (( ! status.equals("IN-SERVICE")) && (! status.equals("GFP-DATA.CONFIG_GUI.IN-SERVICE")) ) {
					if(log.isTraceEnabled()) {
						log.info("ERROR: Continue processing alarm: " + alarm.getIdentifier() + " with SeqNum: " + alarm.getCustomFieldValue(GFPFields.SEQNUMBER) +
								" eventhough Device has provision_status = "+status+".");
					}
				}
			}
		}
