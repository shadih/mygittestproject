 ...
 * 
 */
public final class PtpProcess extends ProblemDefault implements
		ProblemInterface {
	private static Logger log = LoggerFactory.getLogger(PtpProcess.class);
...
", alarm be_time_stamp = " + alarm.getCustomFieldValue("be_time_stamp"));
		...
		{
			// don't suppress it. it needs to send out
			ret = false;
		}
		else
		{
	...
	public boolean isMatchingTriggerAlarmCriteria(Alarm a) throws Exception {
		...
		else
		{
			...
			String watchdogDesc = "CFMPtp watch dog.";
			Util.setTriggerWatch(alarm, CienaAlarm.class, "cfmPtpCallBack", after, watchdogDesc);
			ret = true;
		}
